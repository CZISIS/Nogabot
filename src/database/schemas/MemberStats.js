...
module.exports = {
    ...
    // הוספת הפונקציה לדרוג השיחות
    getVoiceLb: async (guildId, limit = 10) => {
        return Model.aggregate([
            { $match: { guild_id: guildId, "voice.time": { $gt: 0 } } }, // בדוק אם הזמן גדול מאפס
            {
                $project: {
                    member_id: "$member_id",
                    time: "$voice.time",
            ...
            { $sort: { time: -1 } }, // מיין לפי זמן בחדרי שיחות
            { $limit: limit }, // הגדר את מספר התוצאות
        ]);
    },
};
