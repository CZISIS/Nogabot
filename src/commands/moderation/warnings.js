...
async function listWarnings(target, { guildId }) {
    ...
    const acc = warnings.map((warning, i) => `${i + 1}. ${warning.reason} [By ${guild.users.get(warning.admin.id).user.tag} ]`).join("\n");
...
async function listWarnings(target, { guildId }) {
    ...
    // המרת האזהרות למחרוזת, תוך שימוש ב-fetch כדי למשוך את המידע על המנהל
    const acc = await Promise.all(warnings.map(async (warning, i) => {
        const adminMember = await target.guild.members.fetch(warning.admin.id);
        return `${i + 1}. ${warning.reason} [By ${adminMember.user.username}]`;
    }));

    const embed = new EmbedBuilder({
        author: { name: `${target.user.username}'s warnings` },
        description: acc.join("\n"), // חיבור התוצאות למחרוזת
    });
