...
module.exports = {
 ...
 /*   slashCommand: {
    ...
    async messageRun(message, args) {
        ...
        let datausers = 0;
        client.guilds.cache.forEach(x => datausers += x.memberCount);
        ...
        const buildEmbed = () => {
            ...
            const embed = new EmbedBuilder()
                ...
                .setAuthor({ name: `Total Members: ${datausers} - ${client.guilds.cache.size} Guilds` }) // עדכון ה-author
                .setFooter({ text: `${match ? "Matched" : "Total"} Servers: ${total} • Page ${currentPage} of ${totalPages}` });

            // בניית התיאור עם השרתים
            let description = '';
            for (let i = start; i < end; i++) {
                const server = servers[i];
                description += `\`${i + 1}\` - **${server.name}** • \`${server.memberCount}\` Users\n`; // הוספת מספר שרת
            }
            embed.setDescription(description); // הוספת התיאור ל-embed
