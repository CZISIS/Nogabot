...
module.exports = {
    ...
    slashCommand: {
        ...
        options: [
            ...
            {
                name: "user",
                description: "the user to whose avatar the generator needs to be applied",
                type: ApplicationCommandOptionType.User,
                required: false,
            },
            {
                name: "link",
                description: "the image link to which the generator needs to be applied",
                type: ApplicationCommandOptionType.String,
                required: false,
            },
        ],
    ...
    async messageRun(message, args, data) {
        // קח את התמונה מההודעה או מהמשתמש
        ...
        const generator = args[0]; // קח את סוג הגנרטור מהפקודה

        // בדוק אם הגנרטור חוקי
        if (!availableGenerators.includes(generator)) {
            return message.safeReply(`Invalid generator specified. Available generators: ${availableGenerators.join(", ")}`);
        }
