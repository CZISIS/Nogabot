...
module.exports = {
    ...
    async messageRun(message, args, data) {
        // אם לא נשלחה תמונה או אם המשתמש לא תייג מישהו, נשתמש בתמונה של שולח ההודעה
        ...
        const filter = args[0]; // הפילטר הראשון מהפקודה

        // בדוק אם הפילטר חוקי
        if (!availableFilters.includes(filter)) {
            return message.safeReply(`Invalid filter specified. Available filters: ${availableFilters.join(", ")}`);
        }
    ...
    async interactionRun(interaction) {
        ...
        console.log(process.env.STRANGE_API_KEY);
