  ...
  constructor() {
    super({
      intents: [
        ...
        GatewayIntentBits.DirectMessages, // נדרש כדי לקבל הודעות DM
    ...
    this.joinLeaveWebhook = process.env.JOIN_LEAVE_LOGS
      ? new WebhookClient({ url: process.env.JOIN_LEAVE_LOGS })
          : undefined;
      this.nogalogs = process.env.NOGA_LOGS
          ? new WebhookClient({ url: process.env.NOGA_LOGS })
          : undefined;
   ...
   */
    loadCommand(cmd) {
      /*  if (cmd.category === "ADMIN") {
            console.log(`Admin command loaded: ${cmd.name}, ${CommandCategory[cmd.category]?.enabled} `);
        }*/
    ...
    if (cmd.command?.enabled) {
      ...
      this.commandIndex.set(cmd.name.toLowerCase(), index);
       ...
       /* if (cmd.name === 'counter') {
            console.log("Counter command loaded");
        }*/
    } else {
